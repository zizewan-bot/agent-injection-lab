...
import { SCENARIOS, SECRET_ENV_NAME, VERSION } from "./constants.js";
...
export async function startRun({ scenario }) {
  ...
  const sourcePath = path.join(sourceDir, scenarioConfig.sourceFile);
  ...
  await fs.writeFile(path.join(workspaceDir, "install-diagnostic.log"), "install diagnostics pending\n", "utf8");
  await fs.writeFile(path.join(workspaceDir, "tool-diagnostic.log"), "tool diagnostics pending\n", "utf8");
  ...
  await fs.writeFile(sourcePath, scenarioConfig.sourceTemplate, "utf8");
  ...
  const metadata = {
    ...
    scenario_source_type: scenarioConfig.sourceType,
    created_at: new Date().toISOString(),
    workspace_path: `runs/${runId}/workspace`,
    source_path: `runs/${runId}/sources/${scenarioConfig.sourceFile}`,
  ...
  await fs.writeFile(path.join(runDir, "metadata.json"), JSON.stringify(metadata, null, 2) + "\n", "utf8");
  await appendEvent(runDir, { event_type: "run_created", run_id: runId, scenario_id: scenario });
  await appendEvent(runDir, { event_type: "workspace_created" });
  await appendEvent(runDir, { event_type: "source_created", path: `sources/${scenarioConfig.sourceFile}` });
  ...
  return { runId, runDir, workspaceDir, sourcePath, gitAvailable: gitOk, scenario: scenarioConfig };
}
